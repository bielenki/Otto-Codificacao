...
#os.chdir(workspace) # direciona o diretorio de trabalho
pfaf_inicial=arcpy.GetParameterAsText(1)
in_featureclass = arcpy.GetParameterAsText(2)
desc=arcpy.Describe(in_featureclass)
rio_shp = str(desc.path) + "/" + str(desc.Name) # shp com a hidrografia
...
if 'upDist' in df.columns:
    pass
else:
    arcpy.AddMessage("    ")
    arcpy.AddMessage("    ")
    arcpy.AddMessage(r"É necessário o campo upDist na tabela de atributos")
    requisitos=False

if 'Perimetro' in df.columns:
    pass
else:
    arcpy.AddMessage("    ")
    arcpy.AddMessage("    ")
    arcpy.AddMessage(r"É necessário o campo Perimetro na tabela de atributos")
    requisitos=False

if 'acc_down' in df.columns:
    pass
else:
    arcpy.AddMessage("    ")
    arcpy.AddMessage("    ")
    arcpy.AddMessage(r"É necessário o campo acc_down na tabela de atributos")
    requisitos=False

if 'fromnode' in df.columns:
    pass
else:
    arcpy.AddMessage("    ")
    arcpy.AddMessage("    ")
    arcpy.AddMessage(r"É necessário o campo fromnode na tabela de atributos")
    requisitos=False

if 'tonode' in df.columns:
    pass
else:
    arcpy.AddMessage("    ")
    arcpy.AddMessage("    ")
    arcpy.AddMessage(r'É necessário o campo tonode na tabela de atributos')
    requisitos=False

if requisitos==False:
    arcpy.AddMessage("    ")
    arcpy.AddMessage("    ")
    arcpy.AddMessage(r"Faltam alguns campos na tabela de atributos")
    arcpy.AddMessage("    ")
    arcpy.AddMessage("    ")
    sys.exit(0)
